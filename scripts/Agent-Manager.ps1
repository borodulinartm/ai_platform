...
function Stop-Hermes {
    ...
    if ($p) { $p | Stop-Process -Force; Write-Host '  Hermes Desktop stopped' -ForegroundColor Green }
...
function Stop-JiuwenSwarm {
    ...
    if ($p) { $p | Stop-Process -Force; Write-Host '  JiuwenSwarm services stopped' -ForegroundColor Green }
...
function Stop-OpenClaw {
    $p = Get-Process -Name 'openclaw','node' -ErrorAction SilentlyContinue
    if ($p) { $p | Stop-Process -Force; Write-Host '  OpenClaw stopped' -ForegroundColor Green }
